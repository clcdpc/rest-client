...
namespace Clc.Rest
{
    public abstract class RestClient : IRestClient
    {
        ...
        public virtual Task<T> FormatResponseAsync<T>(HttpResponseMessage response, string content, CancellationToken cancellationToken = default)
        {
            ...
            {
                var preDeserializedContent = PreDeserialize(content);

                if (typeof(T) == typeof(string))
                {
                    output = (T)Convert.ChangeType(preDeserializedContent, typeof(T));
                }
                ...
                {
                    output = Deserializer.Deserialize<T>(preDeserializedContent);
                }
            }
