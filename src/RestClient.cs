        ...
        protected virtual HttpRequestMessage AddParameters(RestRequest request, HttpRequestMessage httpRequest)
        {
            ...
            if (request.Method != HttpMethod.Post)
            {
                ...
                if (nonEmptyParameters.Any())
                {
                    ...
                    var querySuffix = string.Join("&", nonEmptyParameters);

                    if (!requestUri.IsAbsoluteUri)
                    {
                        var separator = requestUri.OriginalString.Contains("?") ? "&" : "?";
                        httpRequest.RequestUri = new Uri($"{requestUri.OriginalString}{separator}{querySuffix}", UriKind.Relative);
                    }
                    else
                    {
                        ...
                        var separator = string.IsNullOrEmpty(existingQuery) || existingQuery == "?" ? string.Empty : "&";
                        var existingQueryWithoutPrefix = string.IsNullOrEmpty(existingQuery) ? string.Empty : existingQuery.TrimStart('?');
                        uriBuilder.Query = $"{existingQueryWithoutPrefix}{separator}{querySuffix}";

                        var rebuiltUri = uriBuilder.Uri.GetLeftPart(UriPartial.Path);
                        var rebuiltQuery = uriBuilder.Query;
                        var rebuiltFragment = string.IsNullOrEmpty(fragment) ? string.Empty : fragment;
                        httpRequest.RequestUri = new Uri($"{rebuiltUri}{rebuiltQuery}{rebuiltFragment}", UriKind.Absolute);
                    }
                }
            }
