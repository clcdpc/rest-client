...
If you need synchronous call-site behavior, explicitly block where you call the client (for example `ExecuteAsync(...).Result` or `ExecuteAsync(...).GetAwaiter().GetResult()`).
The library no longer provides synchronous execution wrappers.

Removed in this alpha:

- synchronous `Execute<T>(RestRequest request)` (execution API is now async-only)
