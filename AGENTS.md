...
For async APIs, follow the current release policy: 3.0 execution APIs are async-only. Do not add synchronous Execute<T> wrappers unless a task explicitly requests reintroducing them. Do not add sync-over-async helpers that call `.Result`, `.Wait()`, or `GetAwaiter().GetResult()` on tasks in library code. Content should be read asynchronously by `ExecuteAsync` and passed as a string to `HttpResponse`/formatters.
