...
The v3 alpha library targets **`net8.0` only** and requires **.NET 8+**.

Use platform APIs available on .NET 8 in the library project. If multi-targeting is introduced in the future, any framework-specific behavior must be guarded appropriately and documented.
...
- Do not introduce framework assumptions inconsistent with `net8.0`-only support unless the task explicitly requests another support change.
