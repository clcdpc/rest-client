...
- `QueryParameters` are always URL query values for any HTTP method.
...
- Use `RestRequest.PostForm(...)` for `application/x-www-form-urlencoded` request content.
...
Add tests for any change involving `Body`, `QueryParameters`, URL building, query strings, headers, or authentication.
