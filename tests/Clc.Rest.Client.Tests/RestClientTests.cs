...
public class RestClientTests
{
    ...
    public async Task ExecuteAsync_Absolute_Url_With_Parameters_Appends_Query_String()
    {
        ...
        var response = await client.ExecuteAsync<string>(new RestRequest(HttpMethod.Get, "https://other.example.com/items?existing=true", parameters: new Dictionary<string, string>
        {
            ["q"] = "hello world"
        }), TestContext.CancellationToken);
    ...
    public void Public_Api_Does_Not_Expose_Sync_Execution_Or_Sync_Content_Reads()
    {
        ...
        var httpResponsePublicMethods = typeof(HttpResponse).GetMethods(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Static);
        ...
        Assert.IsNull(httpResponseConstructors.SingleOrDefault(c =>
            c.GetParameters().Length == 1
            && c.GetParameters()[0].ParameterType == typeof(HttpResponseMessage)));
        Assert.IsNull(httpResponsePublicMethods.SingleOrDefault(m => m.Name == "ReadContentSynchronously"));
    }
