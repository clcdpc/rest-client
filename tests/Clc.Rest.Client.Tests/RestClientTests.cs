...
public class RestClientTests
{
    ...
    [TestMethod]
    public void RestRequest_Factories_Create_Expected_Shape()
    {
        var query = new Dictionary<string, string> { ["q"] = "1" };
        var content = new StringContent("x");

        var get = RestRequest.Get("/g", query);
        var del = RestRequest.Delete("/d", query);
        var post = RestRequest.Post("/p", new { A = 1 }, query);
        var put = RestRequest.Put("/u", new { A = 1 }, query);
        var patch = RestRequest.Patch("/pa", new { A = 1 }, query);
        var form = RestRequest.PostForm("/f", new Dictionary<string, string> { ["a"] = "b" }, query);
        var withContent = RestRequest.WithContent(HttpMethod.Put, "/wc", content, query);
        var create = RestRequest.Create(new HttpMethod("MERGE"), null, new { A = 1 }, null);

        Assert.AreEqual(HttpMethod.Get, get.Method);
        Assert.AreEqual(HttpMethod.Delete, del.Method);
        Assert.AreEqual(HttpMethod.Post, post.Method);
        Assert.AreEqual(HttpMethod.Put, put.Method);
        Assert.AreEqual("PATCH", patch.Method.Method);
        Assert.AreEqual(HttpMethod.Post, form.Method);
        Assert.AreEqual(content, withContent.Content);
        Assert.AreEqual("MERGE", create.Method.Method);
        Assert.AreEqual(string.Empty, create.Path);
        Assert.IsNotNull(create.QueryParameters);
    }
    ...
    public async Task ExecuteAsync_Uses_Post_Factory_Request_With_Body_And_QueryParameters()
    {
        ...
        Assert.AreEqual("application/json; charset=utf-8", handler.LastRequest.Content.Headers.ContentType!.ToString());
        Assert.Contains("?a=b", handler.LastRequest.RequestUri!.AbsoluteUri);
    }
    ...
    public async Task ExecuteAsync_MethodUrl_Sends_Absolute_Url_When_BaseUrl_And_PathPrefix_Are_Set()
    {
        ...
        var response = await client.ExecuteAsync<string>(new RestRequest(HttpMethod.Post, "https://other.example.com/items"), TestContext.CancellationToken);
